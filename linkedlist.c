...
//Declaration of all the functions used in SLL's
...
// Begining the main function - the Main program starts here 
int main()
{
...
void Insertbegin(struct node **head_Addr, int value)
{
  ...
  {
    newnode->next=*head_Addr; //*head_Addr = head = &node ; newnode->next = head
    *head_Addr=newnode; //head = newnode
  }
}

//Inserting a node after a given node
void Insertafter(struct node **head_Addr, int value,int element) //element is the data value after which we insert new node
{
  //Create a node and initialize
     struct node *newnode=(struct node*)malloc(sizeof(struct node));
    struct node *key;
